...
//
	...
	public Product(){
		...
		WHERE = "";
		Sel = "SELECT (id) AS id,(name) AS name,(date) AS date,(time) AS time,(age) AS age,(address) AS address,() AS ,(rowid) AS rowid,(serialID) AS serialID,(money) AS money,(memo) AS memo FROM Product";
		Ins = "INSERT INTO Product(id,name,date,time,age,address,,rowid,serialID,money,memo) VALUES(";
		Upd = "UPDATE Product SET ";
		Del = "DELETE FROM Product";
		rcnt = 0;
	}
	
	public Product(Connection con_){
		con = con_;
		try{
	...
	public void execSetResult(ResultSet rs_ ) throws SQLException {
		...
		try{
			while( rs_.next() ){
				Product  rslt_Product = new Product();
				rslt_Product.setId(rs_.getString(1));
				rslt_Product.setName(rs_.getString(2));
				rslt_Product.setDate(rs_.getString(3));
				rslt_Product.setTime(rs_.getString(4));
				rslt_Product.setAge(rs_.getString(5));
				rslt_Product.setAddress(rs_.getString(6));
				rslt_Product.set(rs_.getString(7));
				rslt_Product.setRowid(rs_.getString(8));
				rslt_Product.setSerialID(rs_.getString(9));
				rslt_Product.setMoney(rs_.getString(10));
				rslt_Product.setMemo(rs_.getString(11));
				rsltvec_.addElement(rslt_Product);
			}
...
//
	...
	public final Product exeSelect(String where_){
		where_ = where_.trim();
